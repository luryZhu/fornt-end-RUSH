...
简言之，HOC是一种组件的设计模式，HOC接受一个组件和额外的参数（如果需要），返回一个新的组件。HOC 是==纯函数==，没有副作用。
...
如果一个表单组件没有value props（单选和复选按钮对应的是checked props）时，就可以称为非受控组件。在非受控组件中，可以使用一个**ref**来从DOM获得表单值。而不是为每个状态更新编写一个事件处理程序。
...
**总结：**页面中所有输入类的DOM如果是**现用现取**的称为**非受控**组件，而通过setState将输入的值维护到了state中，**需要时再从state中取出**，这里的数据就受到了state的控制，称为**受控组件**。
